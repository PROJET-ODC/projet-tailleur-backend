...
class ClientController {
    ...
    //         where: {
    //             follower_id: parseInt(compteId), // Assurez-vous que compteId est converti en nombre si nécessaire
    //         },
    //         include: {
    //             followed: true, // Inclure les informations de l'utilisateur suivi
    //         },
    //     });
    
    //     // Filtrer les comptes actifs suivis
    //     const myFollowersCompte = myFollowers
    //         .filter(follow => follow.followed?.etat === 'active') // Assurez-vous que 'active' est la valeur correcte
    //         .map(follow => follow.followed_id); // Harmoniser le nom du champ
    
    //     // Récupération des tailleurs pour les comptes suivis
    //     const myFollowersTailleur = await prisma.tailleur.findMany({
    //         where: {
    //             compte_id: { in: myFollowersCompte }, // Assurez-vous que le champ est correctement nommé
    //         },
    //     });
    
    //     // Liste des identifiants des tailleurs
    //     const myFollowersTailleurIds = myFollowersTailleur.map(tailleur => tailleur.id);
    
    //     // Récupération des publications des tailleurs suivis
    //     const myFollowersPost = await prisma.post.findMany({
    //         where: {
    //             tailleur_id: { in: myFollowersTailleurIds }, // Assurez-vous que 'tailleur_id' est le champ correct
    //         },
    //     });
    ...
    //         // Récupération des statuts des tailleurs suivis
    ...
    //             const oneDayAgo = new Date(now - twentyFourHoursInMs);
    
    //             // Correction: Assurez-vous que `compteId` est correct pour la recherche
    ...
    //             // Correction: Utilisez des conditions correctes pour éviter les erreurs lorsque `tailleur?.id` est `undefined`
    ...
    //             // Correction: Assurez-vous que `getMyFollowersPost` et `getMyFollowersRecentStatus` sont correctement définis
    ...
    //         }
    //     } catch (err) {
    //         console.error('Error fetching news feed:', err); // Ajoutez des logs pour le débogage
    //         return res.status(500).json({ message: 'Erreur interne du serveur', status: 'KO' });
    //     }
    // };
    ...
    //         if (!idUser) {
    //             return res.status(400).json({ message: 'Identifiant utilisateur manquant', status: 'KO' });
    //         }
    ...
    //         if (!compte.user_id) {
    //             return res.status(404).json({ message: 'ID utilisateur non défini dans le compte', status: 'KO' });
    //         }
    
    //         const user = await prisma.user.findUnique({ where: { id: compte.user_id } });
    //         if (!user) {
    ...
    //         if (compte.role === 'tailleur') {
    //             const tailleur = await prisma.tailleur.findUnique({ where: { compte_id: compte.id } });
    //             if (tailleur) {
    ...
    //             const client = await prisma.client.findUnique({ where: { compte_id: compte.id } });
    //             if (client && Array.isArray(client.followClientIds) && client.followClientIds.length > 0) {
    ...
    //                     const compteSuivi = await prisma.compte.findUnique({ where: { id: tailleur.compte_id } });
