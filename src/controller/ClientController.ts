...
class ClientController {
    ...
    async getCompteNoFollowed(req: ControllerRequest, res: Response) {
        try {
            const idCompte = parseInt(req.id as string, 10);
    
            // Requête pour récupérer tous les comptes qui ne sont pas suivis par l'utilisateur connecté (follower_id)
            const comptesNoFollowed = await prisma.compte.findMany({
                where: {
                    // Exclure les comptes qui sont déjà suivis par l'utilisateur connecté
                    id: {
                        notIn: (
                    ...
                    // Assurer que le compte est actif
                    etat: 'actif',
                },
                include: {
            ...
            // Vérifier si des comptes non suivis existent
            if (comptesNoFollowed.length === 0) {
                return res.json({ comptesNoFollowed: [], message: 'Aucun compte non suivi trouvé', status: 'OK' });
            }
    
            return res.json({ comptesNoFollowed, message: 'Comptes non suivis récupérés avec succès', status: 'OK' });
        } catch (error) {
            // Gestion des erreurs
            if (error instanceof Error) {
                return res.status(500).json({ message: 'Erreur lors de la récupération des comptes non suivis', error: error.message });
            }
        }
    }
    ...
    async getUserProfile(req: ControllerRequest, res: Response) {
    try {
        const id = parseInt(req.params.id); // Récupérer l'ID de l'utilisateur à partir des paramètres de la requête
        ...
        console.log('ID récupéré:', id); // Log de l'ID pour vérification
        ...
        if (compte.role === 'tailleur') {
            ...
            if (!tailleur) {
                return res.status(404).json({ message: 'Profil de tailleur non trouvé', status: 'KO' });
            }
            ...
            const totalDesNotes = notes.reduce((somme, note) => {
                return somme + parseFloat(note.note);
            }, 0);
            ...
            return res.json({
                ...
                posts,
                noteToShow,
                message: 'Profil de tailleur retrouvé',
                status: 'OK'
            });
        }
        ...
        if (!client) {
            return res.status(404).json({ message: 'Profil de client non trouvé', status: 'KO' });
        }
        ...
        return res.json({
            ...
            user,
            compte,
            message: 'Profil de client retrouvé',
            status: 'OK'
        });
    } catch (error) {
        console.error('Erreur lors de la récupération du profil:', error); // Log de l'erreur
        return res.status(500).json({ message: 'Erreur interne du serveur', status: 'KO' });
    }
}
